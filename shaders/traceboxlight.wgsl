...
@group(0) @binding(5) var inSampler: sampler;

//var shadingModel : i32;
...
fn boxSpecularColor() -> vec4f {
  return vec4f(0.8,0.8,0.8,1); // specular light is white
}

fn boxAmbientColor() -> vec4f {
...
fn boxNormal(idx: i32) -> vec3f {
  ...
  switch(idx) {
    ...
    case 1: { //back
      return vec3f(0, 0, -1); //adjusted sign of z value
    }
    case 2: { //left
      return vec3f(-1, 0, 0); //adjusted sign of x value
    }
    case 3: { //right
      return vec3f(1, 0, 0);
    }
    ...
    case 5: { //down
      return vec3f(0, -1, 0); // adjusted sign of y value
    }
    default: {
      return vec3f(0, 0, 0);
    }
...
struct LightInfo {
  ...
  reflectedLightDir: vec3f // the reflected light direction
}
...
fn getLightInfo(lightPos: vec3f, lightDir: vec3f, hitPoint: vec3f, objectNormal: vec3f) -> LightInfo {
  ...
  var lightDirection = normalize(lightDir);
  // then, compute the distance between the light source and the hit point
  var dist = length(hitPoint - lightPos);
  var factor = light.attenuation[0] + dist * light.attenuation[1] + dist * dist * light.attenuation[2];
  var viewDirection = normalize(hitPoint - lightPos);

  var out: LightInfo;


  var lightType = light.params[2]; // 0 is default. 1 = Spot, 2 = Directional, 3 = Point
  if (lightType == 1) { //Spotlight
      
      let cutoff = light.params[0];
      let cutoffdotproduct = maxdot(lightDirection, viewDirection);

      let dropoff = light.params[1];
      if(cutoffdotproduct >= cutoff) {
        intensity *= pow(cutoffdotproduct, dropoff);
      }
      else {
        intensity *= 0;
      }
      factor = light.attenuation[0] + dist * light.attenuation[1] + dist * dist * light.attenuation[2];
      intensity /= factor;

      out.intensity = intensity;
      out.lightdir = viewDirection;
      out.reflectedLightDir = reflect(out.lightdir, objectNormal);
  }
  ...
  else if (lightType == 3) { //Pointlight
      factor = light.attenuation[0] + dist * light.attenuation[1] + dist * dist * light.attenuation[2];
      intensity /= factor;
...
//wrapper for finding the max of the dot of v1 and v2, or 0
//changed to abs(dot()), MIGHT refactor
fn maxdot(v1 : vec3f, v2 : vec3f) -> f32 {
    return max(dot(v1,v2),0);
    //return abs(dot(v1,v2));
}

//takes starting point, ray direction, & ray hit info and computes the lambertian shading of our box object
fn lambertianShading(spt: vec3f, rdir: vec3f, hitInfo: vec2f) -> vec4f {
    // here, I provide you with the Lambertian shading implementation
      ...
      diffuse *= lightInfo.intensity * maxdot(normal, lightInfo.lightdir);
...
fn phongShading(spt: vec3f, rdir: vec3f, hitInfo: vec2f) -> vec4f {
    
      let emit = boxEmitColor();
      
      var diffuse = boxDiffuseColor(i32(hitInfo.y)); // get the box diffuse property
      
      var specular = boxSpecularColor(); //specular material color
      var ambient = boxAmbientColor(); //ambient material color
      ...
      var shinyFactor = 50.;
      //TODO: +1 point for the sick debug i made on my shader code (on march 18)
      specular *= lightInfo.intensity * pow(maxdot(-lightInfo.lightdir, lightInfo.reflectedLightDir), shinyFactor);

      ambient *= lightInfo.intensity;
      return emit + diffuse + specular + ambient;
      
    
}

//toon shading
fn toonShading(spt: vec3f, rdir: vec3f, hitInfo: vec2f) -> vec4f {
      ...
      let diffuseThreshold = 0.6;
      if (maxdot(normal, -lightInfo.lightdir) < diffuseThreshold) {
        diffuse *= 0.2;
      }
      
      let shinyFactor = 50.;
      if (pow(maxdot(-lightInfo.lightdir, lightInfo.reflectedLightDir), shinyFactor) < 0.5) {
        specular *= 0.0001;
      }
      //ambient *= lightInfo.intensity;
      return emit + diffuse + specular + ambient;
...
//TODO something neat I wanted to mess around with. rgb shading is now quantized into intervals of 0.1.
fn steppedShading(spt: vec3f, rdir: vec3f, hitInfo: vec2f) -> vec4f {
    
      var out : vec4f = phongShading(spt, rdir, hitInfo);
      return floor(out * 10) / 10;
...
fn computeOrthogonalMain(@builtin(global_invocation_id) global_id: vec3u) {
  ...
  if (uv.x < texDim.x && uv.y < texDim.y) {
    ...
    if (hitInfo.x > 0) { // if there is a hit
      color = lambertianShading(spt, rdir, hitInfo);
    }
...
fn computeProjectiveMain(@builtin(global_invocation_id) global_id: vec3u) {
  ...
  let uv = vec2i(global_id.xy);
  let texDim = vec2i(textureDimensions(outTexture));
  let psizeMultiply = 1.0; // i added this one, default: 1.0f
  let uvAdd = 0.5; // default: 0.5f
  let subtractTotal = 1.0; // default: 1.0f
  let focalLength = 1.0; //changes z value to emulate focal length. larger z values to raise focal length. default: 1.0f
  if (uv.x < texDim.x && uv.y < texDim.y) {
    // compute the pixel size
    let psize = (vec2f(2, 2) / cameraPose.res.xy) * psizeMultiply;
    // projective camera ray sent from origin, with ray direction pointing toward each pixel
    var spt = vec3f(0,0,0);
    var rdir = vec3f((f32(uv.x) + uvAdd) * (psize.x) - subtractTotal, (f32(uv.y) + uvAdd) * (psize.y) - subtractTotal, focalLength);
    // apply transformation
    spt = transformPt(spt);
    rdir = transformDir(rdir);
    // compute the intersection to the object
    ...
    var color = vec4f(0.f/255, 56.f/255, 101.f/255, 1.); // Bucknell Blue
    if (hitInfo.x > 0) { // if there is a hit
      color = phongShading(spt, rdir, hitInfo);
    }
    // set the final color to the pixel
    textureStore(outTexture, uv, color); 
